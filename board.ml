...
type board = {
  name: string;
  mutable plane: ((int * int) * color) array
}
...
let get_center_cell pos =
    Array.get board.plane pos

let gridonboard centcoord  =
  let (x,y) = centcoord in
  { value =  4;
    grid = [| ((),)
           |]
  }

let update_board  (t:tile) (pos:int) :board =
    if check_tile_for_fitting t pos then init_board
